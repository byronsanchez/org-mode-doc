...
** My Org Files
   ...
   into one org file per client normally.  An org file becomes the
   collection of projects, notes, etc for a single client.  Clients
   come and go so I can add or drop entire org files from my agenda to
   keep only what is important visible in agenda views.
