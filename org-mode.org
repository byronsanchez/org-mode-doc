...
** Billing clients based on clocked time
   ...
   correctness before billing for the time.

   Billing for time basically boils down to the following steps:
...
#+end_src
   ...
   recording entries that contribute to the clock report.

   Before invoicing for time it is important to make sure your clocked
   time data is correct.  If you have a clocked time with an entry
   that is not closed (ie. it has no end time) then that is a hole in
   your clocked day and it gets counted as zero (0) for time spent on
   the task when generating clock reports.  Counting it as zero is
...
*** Using clock reports to summarize time spent
   ...
   last month with =F12 1 a m left-arrow R=

   This limits the agenda to this one file, shows the agenda for a 
   full month, moves to last month, and generates a clock report.
   Just scroll down to the end of the agenda to see the report.

   I export the agenda to a text file with =C-x C-w XYZ.txt= so I can
   cut and paste the report and save it with the invoice.
...
*** Creating a task estimate with column mode
   ...
   A property called =Effort= records the amount of time I think
...
#+end_src
   ...
   set the estimated effort value with the quick keys =1=-=9=.

   After setting the effort values exit =column mode= with =q=.
   
*** Saving your estimate
*** Reviewing your estimate
** UNFINISHED Reviewing projects and tasks
*** UNFINISHED Column View
#+begin_src emacs-lisp
; Set default column view headings: Task Quoted Estimated_Effort Clock_Summary
(setq org-columns-default-format "%80ITEM(Task) %10Effort(Effort){:} %10CLOCKSUM")
#+end_src
*** UNFINISHED org-clock-display
=C-c C-x C-d=
*** UNFINISHED Dynamic column view clock report
=C-c C-x i RET=
* Archiving
** Archiving Subtrees
...
** UNFINISHED Limit your view to what you are working on
*** UNFINISHED Using =f5=
...
** UNFINISHED Highlight the current agenda line
#+begin_src emacs-lisp
...
* UNFINISHED Other stuff we may not want to include		   :noexport:
