...
#+end_src
   ...
   - to =OPENPO= status
   - to =CLOSEDPO= status (with a note)
   - to =PROJECT= status (with a note)
   - to =PROJDONE= status (with a note)
...
** USB Stick synchronization
   So I recently acquired a Eee PC 1000 HE which now serves as my main road-warrior laptop replacing
   my 6 year old Toshiba Tecra S1.

   I have a git server on my LAN that hosts bare repositories for all of my projects.
   The problem is I have to leave in 5 minutes and want to make sure I have up-to-date copies
   of everything I work on when I take it on the road (without Internet access).

   To solve this I use a USB stick with bare repositories on it for my git repositories.  This
   includes my org-mode repositories as well as any other source code libraries I'm working on.
   
   So just before I leave I mount my USB stick on my workstation and run =usb-git-fetch= then
   =usb-git-push=.  For any repositories that give non-fast-forward errors merge as required
   until =usb-git-push= reports no errors.  Then I grab my Eee PC and my USB stick and leave.

   Later I mount my USB stick in the Eee PC and do =usb-git-fetch= to grab all of the commits
   from my workstation.  I can merge as required for whatever I want to work on when I'm on the
   road knowing I have full up-to-date history of all my git repos on the USB stick.
   ...
   stick guarantees taht all repos are up to date and allows me to be
   completely mobile and not dependent on Internet access.

   I mount my USB stick in a directory =/usb= using the =fstab= entry
