...
** UNFINISHED Limiting time spent on tasks
   ...
   estimated time for a task is reached.

   I use this to limit the amount of time I spend on a task during the
   day.  As an example, I've been working on this document for over
   two months now.  I want to get it finished but I can't just work on
   it solely until it's done because then nothing else gets worked on.
   I want to do a little bit every day I work on it and limit the
   total amount of time I spend helping out with org-mode to an hour a
   day.
   ...
   Task estimates and =org-clock-sound= clock mode line property usage
