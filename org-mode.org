...
=C-M-r p= starts a capture task normally with a function call on the
task line so I can enter =C-x C-e= to lookup the name in =bbdb= and
replace the function call text.  This starts the clock as soon as the
phone rings and I'm free to lookup and replace the caller in bbdb
anytime during or after the call.  Capture mode starts the clock using
the =:clock-in t= setting in the template.
