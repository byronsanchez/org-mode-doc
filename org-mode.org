...
I'm punch-in on a task in =Project X= then the top-level =Project X=
...
changing my default clocking task.  Or course if I get interrupted in
the middle of the day clocking in a task puts time on that task
regardless of the context I'm supposed to be working in.
...
is a new view in the agenda for this -- just his =v c= in the daily 
