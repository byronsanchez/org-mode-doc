...
*** Project Task States
I no longer have explicit =PROJECT= tasks.  Every level 2 task is a project.
...
This has several advantages:
  ...
  - Clocking in the top level project task works
    ...
    without losing project context for the task.  I used to have a
    =PROJECT= keyword for project tasks but whenever I clocked in a
    =PROJECT= task (by mistake or to track miscellaneous time) it
    changes to =STARTED= and was no longer considered a project task
    for stuck project view.  This really wasn't convenient for me.

  - Simple one-level tasks can be marked unstuck by simply giving them a =NEXT= tag
