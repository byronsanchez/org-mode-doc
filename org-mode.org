...
Setting the restriction to the task is less surprising than
...
want to limit the view to part of the subtree.
...
- =N= in the agenda narrows to the current task subtree
- =U= in the agenda narrows to the parent subtree of this task
- =P= in the agenda narrows to the project containing this task
- =W= in the agenda removes the restriction widening the buffer

These same keys work as speed commands in the org-buffer.

Currently regenerating the agenda removes the org-mode restrictions
but hopefully I can find a solution to that.  My first attempt changed
agenda clock reports so they only included the restricted section of
the org-mode file which wasn't very satisfying.
