...
* UNFINISHED Time reporting and tracking
  ...
  over the past month.  This is where I review my clocking data for 
  correctness before billing for the time.

  Billing for time basically boils down to

  - Check that the clock data in complete and correct
  - Generate a clock report to summarize time spent
  - Create an invoice based on the clock data
  - Archive complete tasks so they are out of the way.

    See the [[Archiving]] section for more details
** Check that the clock data in complete and correct
   Before invoicing for time at the end of the month it's important to
   make sure your clocked time data is correct.  If you have a clocked
   time with an entry that is not closed (ie. it has no end time) then
   that is a hole in your clocked timeframe and it gets counted as
   zero (0) for time spent on the task which is almost certainly
   wrong.
