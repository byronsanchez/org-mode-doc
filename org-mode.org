...
** UNFINISHED Create new tasks and clock in when starting something new
   - Interruption occurs
   - Create a quick remember task C-M-r
   - Type the heading 
   - and clock it in C-c C-x C-i
   - and file it C-c C-c
   - Go do it
** UNFINISHED Time reporting and tracking
*** UNFINISHED Finding tasks to clock in
    - Use the clock history C-u C-c C-x C-i
    - Grab something off today's agenda
    - look at =STARTED= tasks (agenda view)
*** UNFINISHED Checking for holes in clocked times
...
** UNFINISHED weekly review process
*** UNFINISHED sorting REFILE tasks and refiling in groups
...
#+end_src
* UNFINISHED Automatic History, Backups, and Synchronization with Git
#+begin_src lisp
