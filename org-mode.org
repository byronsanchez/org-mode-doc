...
#+begin_src emacs-lisp :tangle yes :exports none
  ...
  ;; org-agenda-files in the variable org-mode-user-agenda-files
  ;;
...
=f5= and =S-f5= are bound the functions for narrowing and widening the emacs buffer as defined
below.

We now use:

    - T for C-c / t on the current buffer
    - N narrows to this task subtree
    - U narrows to the immediate parent task subtree without moving
    - P narrows to the parent project subtree without moving
