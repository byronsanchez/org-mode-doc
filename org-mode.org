...
** No Emacs backup files
   ...
   unacceptable to me.  My solution was no more Emacs backup files.

   org-mode can also encrypt entries but I've never looked into that yet.
   This may be a good alternative for some of my existing encrypted files
   but I haven't had the time to explore that yet.
...
** UNFINISHED Git stuff
   ...
   tracked so I can edit files with confidence.  It's easy to go back
   to a previous version or to see exactly what changed since the last
   commit.  This is great when editing configuration files (such as
   apache configrations, bind9 configurations, etc.)

   I find this extremely useful where your edits might break things
   and having git track the changes means if you break it you can just
   go back to the previous version easily.  This is also true for
   package upgrades for software where the upgrade modifies the
   configuration files.

   I have every version of my edits in a local =git= repository.  Yes
   backups still get done religiously.  I set up an automated network
   backup over 10 years ago that is still serving me well today.  All
   of my systems gets daily backups to a network drive.  Weekly these
   are collected and written to DVD iso images in case my machines
   walk off someday.  I'm not going to lose any important data ever
   again.  Once a week I get an email that says 'These ISO images are
   ready to be burned to disk' and all I have to do is write them out.
   Backups take a minimal amount of time currently and I'm really
   happy about that.  Backups that you have to work hard at don't get
   gone.
