...
#+end_src
   ...
   I am using 2 patches on top of the current master branch from
   org-mode to allow hooks to clock in tasks.  These patches are
   5ffc5f6 (Update modeline with effort and task name on re-clock-in,
   2009-12-02) and 3c658bc (Stop generating superfluous clock entries,
   2009-11-27).  These normally get rebased on to of the master branch
   regularly so the SHA1 values will change.  The patches are available 
   through git at [[http://git.norang.ca/?p=org-mode.git;a=shortlog;h=refs/heads/clock-in][git://git.norang.ca/org-mode.git clock-in]].
...
*** Controlling display of leading stars on headlines
    Org-mode has the ability to show or hide the leading stars on
    ...
    I hide leading stars - this gives a cleaner view of headlines
    when you have deep trees in your outline.
