...
I have disabled the =C-c [= and =C-c ]= keys to prevent messing up my
list of directories in the =org-agenda-files= variable.  I just add
and remove directories manually in the customize interface.  Changing
...
state keyword that has at least one subtask with a todo state keyword
and you have a project.  Projects use the same todo keywords as
regular tasks with one exception - one subtask of a project needs to
be marked =NEXT= so the project is not on the stuck projects list.
...
#+begin_src emacs-lisp :tangle yes
  ...
  %?" :clock-in t :clock-resume t)
                                        ("h" "Habit" entry (file "~/git/org/refile.org") "* TODO %?
  %U
  %a
  SCHEDULED: %t
  :PROPERTIES:
...
tasks more then 2 levels deep just to limit the number of displayed
targets.
...
* Custom agenda views
I now have one block agenda view that has everything on it.  I used to have
11 custom agenda views defined and this required a weekly review that visited
most of these agenda views.  Now I'm striving to simplify my layout with 
everything at my fingertips in a single agenda.
...
scheduled dates (planned to work on today show up in the agenda at the
top.    When searching for tasks in the agenda I disable display of
child tasks with the following setting:
...
=someclient.org=:
: #+FILETAGS: SOMECLIENT WORK NORANG
: ...
: * Administration						      :admin:
: ** Organization
: ...
...
clocked in.  The last step in punching is displays the block agenda.
If you selected a task then the agenda is restricted to that project /
task, otherwise you get everything when clocked into the Organization
task.
...
C-i= (or just =I= in the agenda or speed key setting) or =f9 I=.
starts the clock for this task.
...
task defined.  I normally review these in my weekly review and assign
a =NEXT= task to all projects to clear the stuck project list.  This
...
pacakges like ditaa, graphviz, and PlantUML, and others.
...
Server <- Client: Session Initiation
note right: Client requests new session
Server -> Client: Authorization Request
...
I log a note when a deadline is moved and just the original time when
changing a scheduled item to a new date.
