...
* Org-Mode Setup
I have the following setup in my .emacs so org-mode is enabled.  I use org-mode as the
...
#+end_src
** My Org Files
   I have lots of org files.

   I have the following personal org files
   ...
   I have the following remember-mode org-files
   ...
   I have the following work-related org-files (using ficticious
   client names)

   I deal with multiple clients and client projects and separate these
   into one org file per client normally.  Clients come and go so I
   can add or drop entire org files from my agenda to keep only what
   is important visible in agenda views.
   ...
   I do work for ABC Ltd which has multiple clients systems that I
   work on.  I separate the tasks for each client into separate org
   files.  This makes it easy to add or remove clients.
** Agenda Setup
