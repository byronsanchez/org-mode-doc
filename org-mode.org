...
** Refile Setup
   ...
   I don't refile to tasks more then 3 levels deep just to limit the number of displayed targets.
...
#+end_src
   ...
   I just put the cursor on the task and hit =C-c C-w= and enter =SY
   TAB RET= and it's done (since 'System Maintenance' only shows up in
   my norang.org file).  If the entered text matches multiple targets
   a second =TAB= displays all of the matches and all you need to do
   is pick one.  This works great!
** Refiling Tasks
   ...
   (even ones marked in a =done= state).  (Now that all my refile
   tasks are in a single refile.org file... I could just visit the
   file directly but I'm used to =F12 r= for refiling and there really
   is no benefit in doing it another way at this time.)

   =refile.org= has 
   : #+FILETAGS: REFILE
   so every task in the file can be found using this view.

   If there are a few tasks going to the same place (3 or less) I
   refile the first one, then move to the second one and use =C-c C-w
   up-arrow RET= to refile to the same location again.  If more than 3
   tasks are going to the same place I use the agenda bulk refile 
   function to mark multiple tasks (=m=) and then move them all to 
   the same target with =B r=.  
